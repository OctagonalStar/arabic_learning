# 隐私政策（Privacy Policy）

最后更新日期： 2026-09-23

软件名称：Ar学

软件提供方：OctagonalStar （以下简称“我们”）

我们深知隐私对您的重要性。本隐私政策旨在向您清晰、透明地说明，当您使用 Ar学（以下简称“本软件”）时，我们如何处理您的信息。

## 一、我们收集的信息

**我们绝不收集任何您的个人数据。**

本软件设计为本地运行的应用程序，我们未嵌入任何第三方统计分析SDK、未进行任何遥测或崩溃报告自动上传。

您在使用过程中所产生的所有信息，包括但不限于：

- 您学习过的词库词汇及学习进度；
- 您设定的复习计划与时间戳；
- 您自定义添加的本地词库；
- 您的软件设置。

均存储在您的设备本地，我们无法通过互联网访问、读取、备份或传输这些信息。

> 说明：您在首次启动时填写的**用户名**、以及您自行填写的 **WebDAV 地址、用户名与密码**等设置同样仅保存在设备本地，并会随本地导出与 WebDAV 备份一并包含在备份数据中。

## 二、第三方收集的数据

为了实现特定功能，您的客户端在您使用相应功能时会与外部的第三方服务器产生数据交互，这可能涉及部分信息的暴露。具体情况如下：

### 在线词库和TTS模型下载（HTTPS）

当您点击下载在线词库或TTS模型时，您的设备将直接请求 GitHub 的服务器。

在此过程中，GitHub 将获取您的**IP 地址、用户代理（User-Agent）、请求的文件名、其他网络元信息等**，用以完成文件传输和基础的安全审计。

我们无法控制 GitHub 对这些日志的使用。

> 请注意：GitHub总部位于美国，其主体受美国法律管辖。
> 注：您通过软件中`本地词库导入`功能导入的词库不受上述影响。

### WebDAV 数据同步（HTTP / HTTPS）

如果您**主动开启并配置**了 WebDAV 同步功能，您的**本地备份数据**将通过HTTP或HTTPS协议传输至您自行填写地址的第三方服务器。

该备份包含您在本软件中的全部持久化数据，包括：**用户名、软件设置（含您填写的 WebDAV 地址、用户名与密码）、词库与学习进度、复习（FSRS）配置、阅读题、同义词标记及每日推送断点**。

**该数据由您直接发送给您的 WebDAV 服务提供商，我们从未经手、暂存或备份您的这些数据。您是该数据的实际控制者，您应当自行评估并承担您所选 WebDAV 服务商的数据安全能力。**

> 注意：您填写的 WebDAV 地址、用户名与密码会以明文保存在设备本地设置中，并因此一同进入本地导出文件与 WebDAV 备份。请妥善保管您的备份文件。

### 局域网联机（WebRTC / STUN）

当您主动发起联机时，为建立 P2P 连接，您的**本地内网 IP 及公网 IP 信息**将通过 STUN 协议发送给第三方 STUN 服务器。本软件默认使用miwifi和Google提供的公共 STUN 服务。

同时，联机进程启动时您若为客端（非房主）设备，软件将获取您设备上的**当前时间**、本软件版本号、本地词库的特征值发送给房主设备，以消除时差影响、版本匹配和筛选共有词库。

联机进程中，您在联机学习界面中的**进度信息**将会**实时**地通过WebRTC协议传递给您的联机对象，以实现双方进度显示的功能。

在联机进程结束后，您在此次联机学习中的**做题正确数、使用时长**会通过WebRTC协议传递给您的联机对象，以计算双方得分。

当您使用**扫码方式**建立联机时，软件会请求**相机权限**并读取二维码内容（联机口令）。相机画面仅在您的设备本地处理，不会上传；二维码内容仅用于建立本次联机。

本软件设计中，联机进程中所使用的数据**仅临时保存在内存中，不持久化，在用户返回主界面后这些内存将被释放**。

**由于网络环境不确定性，我们不对您使用联机功能可靠性担保。**

> `进度信息` 是指当前完成题目的正误列表，包含有您当前完成数量以及正确和错误题目数量

### AI API阅读题生成(HTTP / HTTPS)

当您使用添加阅读题中的**使用AI API生成**选项并且填写了您的**服务商地址、API密钥和模型名称**后，您的设备将直接使用`POST`请求您所填写的API地址；请求头中包含您的密钥；请求体中包含模型名称及软件生成的相关提示词。

完成请求后，软件会提取并解析AI模型回复内容。若解析成功则其作为生成的阅读题进行持久化，若请求或解析失败，则会回退到设置页面，不持久化任何数据。

本软件**不会**存储有关你的API的任何数据，包含服务商地址、API密钥和模型名称。你需要自行妥善保管这些数据。

使用此功能会使得你填写的API密钥、模型名称以及你设备本身相关网络元信息被发送至你所填写的地址。

### 在线文本转语音（TextReadTTS.com，HTTP / HTTPS）

当您在音频设置中选择**请求 TextReadTTS.com 的语音**后，您要朗读的文本会通过 `GET` 请求发送至 `textreadtts.com`，由其返回音频。

在此过程中，该服务商将获取您的**IP 地址、用户代理及待合成文本**。我们无法控制该服务商对上述信息的使用。

> 若您不希望待朗读文本被发送至第三方，请改用系统 TTS 或本地神经网络语音合成。

### 系统通知与本地后台任务（Android）

开启学习提醒后，软件会请求**通知权限**，并注册约每 30 分钟运行一次的本地后台任务，用于在本地判断是否发送当日学习提醒。

该后台任务**仅读取设备本地设置，不联网、不上传任何数据**。

## 三、信息的存储与安全

由于所有数据均在本地，数据的安全性与您的设备安全直接相关。我们建议您：

- 通过软件内数据文件导出功能，定期备份数据文件；
- 确保您的操作系统和设备处于安全状态。

因设备遗失、损坏、系统重装或软件卸载导致的数据丢失，我们无法恢复且不承担赔偿责任。

### 数据位置

Android端：软件数据存储在`/data`目录下，卸载软件将导致数据被完全清除。

Windows端：软件数据存储在 用户主目录的 `Documents\aribic_learning\` 及 `$Roaming_AppData_Directory\com.example.arabic_learning/` 下。

Linux端：软件数据存储在 `$XDG_DATA_HOME/com.example.arabic_learning/` 下。

Web端：软件数据存储在浏览器 `LocalStorage` 或 `IndexDB` 下，清除浏览器数据或网页数据可能导致数据丢失。

## 四、信息的共享与披露

我们不会与任何第三方共享、出售或出租您的任何数据。

## 五、您的权利

您对您的本地数据拥有完全的控制权：

查看与修改：您可在软件中随时查看和修改您的学习数据；
删除：您可通过系统对应用的管理功能（如 Android 设置中的“清除数据”）、直接卸载软件，或手动删除本地存储目录下的文件，来永久删除您的所有数据。

## 六、隐私政策的变更

我们可能适时修订本隐私政策。

如有重大变更，我们将通过软件更新弹窗，并重新征得您的明确同意。

用户协议`七、条款修改`中的规定对此条款同样生效。

## 七、联系我们

如您对本隐私政策有任何疑问，请通过Github Issue联系我们
